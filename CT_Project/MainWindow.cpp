...
void MainWindow::switchView()
{
    ...
    {  // 如果为空 怎么说明 还未生成 要显示的文件，所以就切换 demo窗口
        if ( 2 == switchViewFlag )
        {
            qDebug() << "show 1";
...
bool MainWindow::viewPointData()
{
    ...
    //读入221021211027_pcl_2.txt数据 并把所有的点映射到 中心为 0，0,0点  边长为1 的 正方体中
    if ( false == pointCloud.read( "BOLUS.txt" ) )
    {
    ...
    else
    {
        ...
        // 只要不是故意的把，显示已生成按钮 点击 几十次 甚至几百次的话，程序一般不会变的很卡到崩溃；
        // 这里做的不是很好，后者可以进行优化；
        pointCloudWidget = new MyGLWidget( this, pointData, 1 );  // 1代表要显示 点云文件
    }
    return true;
...
bool MainWindow::viewStlData()
{
    ...
    {
        // stlWidget->close();
        // delete stlWidget; // 加上这步 不知道为啥 会出错，但是不加的 随着点击次数的增多，程序会占用很多的内存，
        // 经过测试发现，
        //  只要不是故意的把，显示已生成按钮 点击 几十次 甚至几百次的话，程序一般不会变的很卡到崩溃；
        //  这里做的不是很好，后者可以进行优化；
        stlWidget = new MyGLWidget( this, pointData, 2 );  // 2代表要显示 stl文件
